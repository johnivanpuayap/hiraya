...
export default async function StudentAssignmentsPage() {
  ...
  // Get assignments for those classes, plus class names separately
  ...
  const { data: classes } = uniqueClassIds.length > 0
    ? await admin
        .from("classes")
        .select("id, name")
                ...
                <div className="flex items-start justify-between">
                  <div>
                    ...
                    <p className="mt-0.5 text-xs text-text-secondary">
                      {classNameMap.get(assignment.class_id) ?? "Unknown class"} · {assignment.question_count} questions · {assignment.mode}
                    </p>
                    {assignment.deadline && (
                      <p
